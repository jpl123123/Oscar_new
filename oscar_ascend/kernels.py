...
from .config import attention_programs, percentile_selection
...
def _attention_kernel(
    ...
    # One query task per program. Keep the task lookup on device, but avoid
    # nesting a persistent task loop around the mixed Cube/Vector KV loop.
    ...
    logsum = tl.where(denominator > 0, m + tl.log(tl.maximum(denominator, 1.0e-30)), -float("inf"))
...
def attention_partials(query, key, value, history, window, meta, layout, scale, splits, raw):
    ...
    programs = attention_programs(n, meta.max_num_reqs, cfg.queries_per_tile)
    _attention_kernel[(programs, h, splits)](
        query,
        key,
        value,
        history,
